...
class Dremio:
	...
	def get_catalog_entity_by_path(self, path, report_error=True):
		...
		url = self._catalog_url_by_path + path
	...
	def get_catalog_entity_by_id(self, entity_id):
		if entity_id[:7] == 'dremio:':
			...
			url = self._catalog_url + entity_id
	...
	def get_catalog_entity_graph_by_id(self, entity_id, report_error=True):
		url = self._catalog_url + entity_id + '/' + self._graph_url_postfix
	...
	def get_user_by_name(self, username):
		url = self._user_by_name_url + username
	...
	def get_group_by_name(self, groupname):
		url = self._group_by_name_url + groupname
	...
	def refresh_reflections_by_pds_id(self, pds_entity_id, dry_run=True):
		if dry_run:
			...
			return
		url = self._reflections_url + pds_entity_id + self._refresh_reflections_postfix
	...
	def refresh_reflections_by_pds_path(self, pds_entity_path, dry_run=True):
		...
		url = self._catalog_url + pds_entity_id + "/" + self._refresh_reflections_postfix
	...
	def update_wiki(self, catalog_id, wiki, dry_run=True):
		if dry_run:
			...
			return
		url = self._catalog_url + catalog_id + "/collaboration/wiki"
	...
	def update_tag(self, catalog_id, tag, dry_run=True):
		if dry_run:
			...
			return
		url = self._catalog_url + catalog_id + "/collaboration/tag"
	...
	def promote_pds(self, pds_entity, dry_run=True):
		if dry_run:
			...
			return
		url = self._catalog_url + self._encode_http_param(pds_entity['id'])
	...
	def delete_reflection(self, reflection_id, dry_run=True, report_error=True):
		if dry_run:
			...
			return
		url = self._reflection_url + reflection_id
		return self._api_delete(url, source="delete_reflection", report_error = report_error)
